...
function requireValue(name, value = process.env[name]) {
  if (!value || isPlaceholder(value)) {
    throw new Error(`Missing ${name}. Set it in ios/.env (copy .env.template first).`);
  }
  return value;
}
...
const bundleId = requireValue("BUNDLE_ID");
...
const config = {
  ...
  isBrowserStack,
  appDisplayName,
  suiteName: `${appDisplayName} E2E (iOS)`,
  projectName: optionalValue(process.env.BROWSERSTACK_PROJECT_NAME) || `${appDisplayName} iOS`,
  ...
  appId: bundleId,
  bundleId,
  appPath: optionalValue(process.env.APP_PATH),
