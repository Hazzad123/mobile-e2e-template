...
function targetRunName(target) {
  return `Android ${target.osVersion} - ${target.device}`;
}
...
function extractCaseIds(title) {
  return [...title.matchAll(/\bC(\d+)\b/g)].map((match) => Number(match[1]));
}
...
function buildCaseResults(summary, target) {
  ...
  for (const result of summary.results || []) {
    ...
    for (const caseId of caseIds) {
      ...
      if (result.status === "fail") {
        existing.failed++;
        if (result.errorMessage) existing.failures.push(result.errorMessage);
      }
...
function buildResultComment(summary, target, result) {
  const lines = [
    `Automated BrowserStack result for ${target.device} / Android ${target.osVersion}.`,
...
class TestRailPublisher {
  ...
  async ensurePlanEntry(caseIds) {
    ...
    const runs = this.matrix.map((target) => ({
      ...
      description: `Automated BrowserStack E2E run for ${target.device} / Android ${target.osVersion}.`,
      include_all: false,
      case_ids: caseIds,
  ...
  async resolveConfigIds() {
    ...
    const androidGroup = groups.find((group) => normalizeConfigText(group.name) === "android");
    const configs = androidGroup?.configs || [];

    for (const target of this.matrix) {
      const config = findConfigForTarget(configs, target);
      if (!config) {
        throw new Error(
          `No TestRail Android configuration found for ${target.device} / Android ${target.osVersion}. ` +
  ...
  async publishTarget(summary, target) {
    ...
    if (!runId) {
      throw new Error(`No TestRail run id found for ${target.device} / Android ${target.osVersion}.`);
    }
...
function findConfigForTarget(configs, target) {
  ...
  const androidMajor = `android ${majorOsVersion(target.osVersion)}`;
  const candidates = configs.filter((config) => {
    const name = normalizeConfigText(config.name);
    return name.includes(device) && name.includes(androidMajor);
  });
