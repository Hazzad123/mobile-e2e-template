...
const fakeTestRailConfigIds = DEVICES.map((_, index) => 3200 + index);
        ...
        return [
          {
            ...
            configs: DEVICES.map((target, index) => ({
              ...
              name: `${target.device} - Android ${target.osVersion.split(".")[0]} (Browserstack)`,
            })),
          },
        ];
      }
...
async function assertPlanEntryAndResults() {
  ...
  assert.deepStrictEqual(
    ...
    DEVICES.map(({ device, osVersion }) => `Android ${osVersion} - ${device}`),
  );
  ...
  assert.strictEqual(
    ...
    `Android ${DEVICES[0].osVersion} - ${DEVICES[0].device}`,
  );
}
...
async function main() {
  ...
  const multi = buildRequiredCaseIds([{
    results: [{ title: "C101/C202 One automated check maps to multiple cases" }],
  }]);
  assert.deepStrictEqual(multi, [101, 202]);
  assert.throws(
    () => buildRequiredCaseIds([{
