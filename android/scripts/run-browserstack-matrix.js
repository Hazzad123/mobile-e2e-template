...
const DEFAULT_ANDROID_MATRIX = DEVICES
...
function parseMatrix(raw) {
  return raw
    ...
    .map((entry) => {
      ...
      if (!device || !osVersion) {
        throw new Error(
          `Invalid BS_ANDROID_MATRIX entry "${entry}". Use "Device Name@OS Version".`,
        );
      }
    ...
    .map((target) => ({
      ...target,
      id: `android-${safeName(target.osVersion)}-${safeName(target.device)}`,
    }));
}
...
function buildMatrixRunName() {
  ...
  return build ? `Android E2E Regression ${date} #${build}` : `Android E2E Regression ${date}`;
}
...
function buildSharedBuildName() {
  ...
  return `Android ${runLabel} ${new Date().toISOString().slice(0, 10)}`;
}
...
function runDevice(target) {
  ...
  console.log(`[MATRIX] Starting ${target.device} / Android ${target.osVersion}`);
  ...
  return new Promise((resolve) => {
    ...
    // Test Observability so each test gets embedded video/screenshots. Per-device
    // env vars below are read by wdio.conf.js (device/OS/build) and test.js (SUMMARY_ID).
...
async function main() {
  const matrix = parseMatrix(process.env.BS_ANDROID_MATRIX || DEFAULT_ANDROID_MATRIX);
  ...
  matrix.forEach((target, index) => {
    ...
    results.push({
      ...
      failures: (summary?.results || []).filter((result) => result.status === "fail"),
  ...
  try {
    ...
    for (const entry of summaries) {
      if (!entry.summary) {
        throw new Error(`No summary was produced for ${entry.target.device} / Android ${entry.target.osVersion}.`);
      }
  ...
  console.log("\n" + "-".repeat(72));
  console.log("App Automate sessions:");
  const buildUrl = results.find((result) => result.buildUrl)?.buildUrl;
  if (buildUrl) console.log(`  Build: ${buildUrl}`);
  for (const result of results) {
    console.log(`  ${result.device} / Android ${result.osVersion}: ${result.sessionUrl || "(session URL unavailable)"}`);
  }

  const failures = results.filter((result) => result.failures.length > 0);
  if (failures.length > 0) {
    console.log("\nFailures → device → session:");
    for (const result of failures) {
      for (const failure of result.failures) {
        console.log(`  ❌ ${failure.title}${failure.videoTs ? ` (video ~${failure.videoTs})` : ""}`);
        console.log(`     ${result.device} / Android ${result.osVersion}`);
